export default {
  failed: 'Action failed',
  success: 'Action was successful',
  app: {
    title: 'Starlight Tech Radar',
    navigation: 'Navigation',
    homepage: 'Homepage',
    homepage_caption: 'Return to the radar',
    settings: 'Settings',
    settings_caption: 'App preferences',
    created_by: 'Created by',
    with_love: 'with Love',
    toggle_menu: 'Toggle navigation menu',
    close_menu: 'Close navigation menu',
    logo_alt: 'Starlight Tech Radar Logo',
    skip_to_content: 'Skip to main content'
  },
  accessibility: {
    title: 'Accessibility Settings',
    font_size: 'Font Size',
    font_size_caption: 'Adjust base text size',
    open_dyslexic: 'Open Dyslexic',
    open_dyslexic_caption: 'Use font for better readability',
    dark_mode: 'Dark Mode',
    dark_mode_caption: 'Toggle between dark and light themes'
  },
  radar: {
    view_radar: 'Radar',
    radar_view: 'Radar View',
    view_list: 'List',
    list_view: 'List View',
    filterRings: 'Filter Rings',
    searchBlips: 'Search Blips by Name...',
    newOnly: 'New only',
    clearFilters: 'Clear filters',
    noBlipsFilters: 'No Blips for chosen filters...',
    noBlips: 'No Blips configured.',
    licenseLabel: 'License',
    licenseRating: 'Bluebird License Rating',
    quadrants: {
      techniques: 'Techniques',
      platforms: 'Platforms',
      tools: 'Tools',
      languages: 'Languages & Frameworks'
    },
    rings: {
      adopt: 'Adopt',
      trial: 'Trial',
      assess: 'Assess',
      hold: 'Hold'
    },
    blips: {
      new: 'New',
      stable: 'Stable',
      guidance: 'Guidance',
      repository: 'Repository',
      quadrant: 'Quadrant',
      ring: 'Ring',
      close: 'Close',
    },
    feedback: {
      title: 'Feedback for',
      give_feedback: 'Give Feedback',
      type: 'Feedback Type',
      type_placeholder: 'Select feedback type',
      types: {
        positive: 'Positive',
        negative: 'Negative',
        suggestion: 'Suggestion',
        question: 'Question'
      },
      comment: 'Comment',
      comment_placeholder: 'Share your thoughts...',
      submit: 'Send Feedback',
      cancel: 'Cancel',
      success: 'Thank you for your feedback!',
      error: 'Failed to send feedback. Please try again later.'
    }
  },
  settings: {
    language_region: 'Language & Region',
    choose_language: 'Choose your preferred language',
    app_language: 'Application Language'
  },
  errors: {
    not_found: 'Oops. Nothing here...',
    go_home: 'Go Home'
  }
}
